...
def cornersHeuristic(state, problem):
    ...
    distances = [abs(currentPosition[0] - corner[0]) + abs(currentPosition[1] - corner[1])
                          for i, corner in enumerate(corners) if not visitedCorners[i]]
...
def foodHeuristic(state, problem):
    ...
    maxDistance = max(abs(position[0] - food[0]) + abs(position[1] - food[1]) for food in foodList)
